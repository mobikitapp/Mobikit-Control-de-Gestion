...
class OrdenAreaProgresoRepository:
    ...
    @staticmethod
    def get_current_progress(orden_id: int) -> Optional[OrdenAreaProgreso]:
        """Get current area progress for an order"""
        return (db.session.query(OrdenAreaProgreso)
                ...
                .filter_by(orden_fabricacion_id=orden_id, es_actual=True)
                .first())

    @staticmethod
    def get_progress_history(orden_id: int) -> List[OrdenAreaProgreso]:
        """Get complete progress history for an order"""
        return (db.session.query(OrdenAreaProgreso)
                .options(
                    joinedload(OrdenAreaProgreso.area),
                    joinedload(OrdenAreaProgreso.estado),
                ...
                .filter_by(orden_fabricacion_id=orden_id)
