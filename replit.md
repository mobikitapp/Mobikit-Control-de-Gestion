...
## Recent Changes
- **2025-11-03**: **Fixed Planificación y Prioridades module JavaScript error** - Corrected critical error in the weekly load chart generation where JavaScript attempted to serialize SQLAlchemy objects directly. Changed template to use `proyectos_gantt` (pre-serialized data structure) instead of `proyectos` (raw model objects), and updated property access from `of_info.of.fecha_planificada` to `of_info.fecha_planificada` to match the serialized structure. Also resolved type annotation incompatibility in `replit_auth.py` by adding `# type: ignore[override]` comment to suppress false positive from flask-dance BaseStorage interface. These changes eliminate the "'dict object' has no attribute 'of'" error and restore full functionality to the production planning matrix and capacity charts.
