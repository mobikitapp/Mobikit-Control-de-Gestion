...
## Recent Changes
- **November 25, 2025 (Afternoon)**: Fixed production bug where bulk estado change modal did not display area/estado options. Solution: Pre-render estado options server-side (grouped by area) instead of fetching them asynchronously with JavaScript. This ensures the modal always has options available regardless of network conditions or client-side script execution timing.
- **November 25, 2025 (Late)**: Enabled archiving of manufacturing orders when they reach "Despachado" status in Bodega area. Modified `archive_dispatch()` method to accept orders from both Despacho area and Bodega area with Despachado state. Updated UI messaging to clarify that orders can be archived when dispatched.
...
- **Manufacturing Orders (OF)**: Manages production workflows (planned → in_production → QA → finished → delivered) with material tracking, quality control, and time comparison. Includes bulk operations for multi-selection with checkboxes, allowing mass state changes, archiving, and deletion with partial success handling and role-based permissions. Features project grouping view (default) with project-level checkboxes for batch selection and toggleable flat list view. Can be archived when dispatched from either Despacho area or Bodega area with Despachado status.
