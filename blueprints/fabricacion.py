...
@fabricacion_bp.route('/api/areas')
@require_login
def api_areas():
    """API endpoint para obtener todas las áreas"""
    try:
        from models import Area
        areas = db.session.query(Area).filter_by(activo=True).order_by(Area.orden_secuencia).all()
        return jsonify([{
            'id': area.id,
            'nombre': area.nombre,
...
@fabricacion_bp.route('/api/areas/<int:area_id>/estados')
@require_login
def api_estados_area(area_id):
    """API endpoint para obtener estados de un área"""
    try:
        from models import AreaEstado
        estados = (db.session.query(AreaEstado)
                  .filter_by(area_id=area_id, activo=True)
                  .order_by(AreaEstado.orden_en_area)
                  .all())
        ...
        logger.error(f"Error en API estados área {area_id}: {str(e)}")
        return jsonify({'error': 'Error al cargar estados'}), 500

@fabricacion_bp.route('/api/fabricacion/<int:of_id>/estado-actual')
@require_login
def api_estado_actual(of_id):
    """API endpoint para obtener el estado actual de una OF"""
    try:
        of = fabricacion_service.get_orden_fabricacion_by_id(of_id)
        if not of:
            return jsonify({'error': 'Orden no encontrada'}), 404

        progreso_actual = of.area_progreso_actual
        if progreso_actual:
            return jsonify({
                'area': progreso_actual.area.nombre,
                'estado': progreso_actual.estado.nombre,
                'area_id': progreso_actual.area.id,
                'estado_id': progreso_actual.estado.id
            })
        else:
            return jsonify({
                'area': None,
        ...
        logger.error(f"Error obteniendo estado actual OF {of_id}: {str(e)}")
