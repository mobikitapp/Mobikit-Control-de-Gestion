...
@app.route('/configuraciones/limpiar-cache', methods=['POST'])
@login_required
def limpiar_cache():
    """Endpoint para limpiar la caché del sistema"""
    try:
        # Clear any server-side cache
        # Force browser cache refresh by updating timestamps
...
@app.route('/configuraciones/limpiar-datos', methods=['POST'])
@login_required
def limpiar_datos():
    ...
    try:
        ...
        # Execute the cleaning script logic
        with db.engine.connect() as conn:
            with conn.begin():
                # 1. Delete all manufacturing order categories
                conn.execute("DELETE FROM orden_fabricacion_categorias")

                # 2. Delete all manufacturing orders
                conn.execute("DELETE FROM ordenes_fabricacion")

                # 3. Delete all purchase orders
                conn.execute("DELETE FROM ordenes_compra")

                # 4. Delete contract deliveries
                conn.execute("DELETE FROM entregas_contrato")

                # 5. Delete project categories
                conn.execute("DELETE FROM proyecto_categorias")

                # 6. Reset project states to 'active'
                conn.execute("""
                    UPDATE proyectos 
                    SET estado = 'activo', 
                ...
                # 7. Delete associated dispatches
                conn.execute("DELETE FROM despacho_archivos")
                conn.execute("DELETE FROM despachos")

                # 8. Delete related reminders
                conn.execute("DELETE FROM recordatorios WHERE tipo IN ('despacho', 'proyecto')")

                # 9. Clean related audit records
                conn.execute("""
        ...
        return jsonify({
            'success': True,
            'message': 'Datos limpiados exitosamente. Los proyectos permanecen activos pero sin órdenes asociadas.'
        })
